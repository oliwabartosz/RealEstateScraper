...
list_of_id_in_gpt_database = [(item['id'], item.get('status', None)) for item in offers_gpt_data]
print(list_of_id_in_gpt_database[400])
exit(1)

result = {}

for offer_record in offers_data:
    if list_of_id_in_gpt_database[1]: #if status is not None
